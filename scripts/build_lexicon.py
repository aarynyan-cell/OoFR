...
LEXICON_VERSION = "2025.11.23+lexique383+ipa-dict+lefff"
LEXICON_FORMAT = "chunked-tsv-1"
CHUNK_TARGET_SIZE = 40000
...
def encode_tsv_field(value: str) -> str:
    return (
        str(value)
        .replace("\\", "\\\\")
        .replace("\t", "\\t")
...
def chunk_lines(lines: list[str], target_size: int = CHUNK_TARGET_SIZE) -> list[str]:
    chunks: list[str] = []
    current: list[str] = []
    current_size = 0
...
def js_string_array(chunks: list[str]) -> str:
    if not chunks:
        return "[]"
    items = ",\n".join(
...
def write_lexicon(entries: dict[str, dict[str, str]], form_lemmas: dict[str, str]) -> None:
    rows = [
        "\t".join(
            [
                encode_tsv_field(key),
                encode_tsv_field(item["word"]),
                encode_tsv_field(item.get("ipa", "")),
                encode_tsv_field(item["zh"]),
            ]
        )
        for key, item in sorted(entries.items(), key=lambda pair: pair[0])
    ]
    form_rows = [
        "\t".join([encode_tsv_field(form), encode_tsv_field(lemma)])
        for form, lemma in sorted(form_lemmas.items(), key=lambda pair: pair[0])
        if form != lemma and lemma in entries
    ]
    lexicon_chunks = js_string_array(chunk_lines(rows))
    form_chunks = js_string_array(chunk_lines(form_rows))
    content = (
        ...
        f"   Format: {LEXICON_FORMAT}; avoids huge array literals for iOS Safari.\n"
        "   Do not edit this file by hand. */\n"
        "(function(){\n"
        '  "use strict";\n'
        f"  var lexiconChunks={lexicon_chunks};\n"
        f"  var formChunks={form_chunks};\n"
        "  var lexicon=Object.create(null);\n"
        "  var forms=Object.create(null);\n"
        "  var entryCount=0;\n"
        "  var formCount=0;\n"
        "  function decodeField(value){\n"
        "    return value.replace(/\\\\([tnr\\\\])/g,function(_match,ch){\n"
        "      if(ch==='t') return '\\t';\n"
        "      if(ch==='n') return '\\n';\n"
        "      if(ch==='r') return '\\r';\n"
        "      return '\\\\';\n"
        "    });\n"
        "  }\n"
        "  function readChunks(chunks,fieldCount,visitor){\n"
        ...
        "          parts[partIndex]=decodeField(parts[partIndex]);\n"
        "        }\n"
        "        visitor(parts);\n"
        "      }\n"
        "    }\n"
        "  }\n"
        "  readChunks(lexiconChunks,4,function(parts){\n"
        "    lexicon[parts[0]]={word:parts[1],ipa:parts[2],zh:parts[3]};\n"
        "    entryCount+=1;\n"
        "  });\n"
        "  readChunks(formChunks,2,function(parts){\n"
        "    forms[parts[0]]=parts[1];\n"
        "    formCount+=1;\n"
        "  });\n"
        "  window.OOFR_LEXICON=lexicon;\n"
        "  window.OOFR_FORM_LEMMAS=forms;\n"
        "  window.OOFR_LEXICON_ROWS={length:entryCount};\n"
        "  window.OOFR_FORM_LEMMA_ROWS={length:formCount};\n"
        f"  window.OOFR_LEXICON_META={{version:'{LEXICON_VERSION}',format:'{LEXICON_FORMAT}',entries:entryCount,forms:formCount}};\n"
        "}());\n"
    )
