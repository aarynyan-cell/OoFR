...
def main() -> None:
    ...
    for config in selected:
        ...
        write_package(config, package)
        update_manifest_package(manifest, config, package)
        print(
            f"  wrote {config.pair}: "
...
def write_package(config: PackConfig, package: dict[str, Any]) -> None:
    target = OUT_DIR / f"{config.pair}.json"
    target.write_text(json.dumps(package, ensure_ascii=False, separators=(",", ":")), encoding="utf-8")
...
def load_manifest() -> dict[str, Any]:
    ...
    return {
        ...
        "version": "1.0.0-alpha",
        "updatedAt": "",
        "notes": "Generated by scripts/build_lexicons.py.",
        "packages": [],
    }


def update_manifest_package(manifest: dict[str, Any], config: PackConfig, package: dict[str, Any]) -> None:
